...
const counterSlice = createSlice({
	...
	reducers: {
		...
		addCounterByOther(
			state,
			action: PayloadAction<{ targetId: string; otherId: string }>,
		) {
			const target = state.counters.find(
				(ct) => ct.id === action.payload.targetId,
			);
			const other = state.counters.find(
				(ct) => ct.id === action.payload.otherId,
			);
			if (target && other) {
				target.value = target.value + other.value;
			}
		},
